                    ...
                    <div class="product2">
                        <div class="card-space">
                            <div class="cards">
                                ...
                                <button><a href="">veiw more</a></button>
                            </div>
                            <div class="cards">
                                <div class="">
...
if(isset($_POST['submit'])){
    ...
    if($pre->execute(['name' => $name, 'email' => $email, 'phone' => $phone, 'message' => $massage, 'city' => $city, 'subject' => $subject])){
        echo "<script>alert('Data Inserted Successfully!'); window.location=('product.php?status=success');</script>";
        exit(); 
    } 
    }
    
?>
                <form action="" method="post"><span id="cross">❌</span>
                <h4>enquiry form</h4>

                    <input type="text" placeholder="Name:" pattern="[A-Za-z ]{3,}" title="Naam mein sirf letters hone chahiye" name="name" required>
                    <input type="email"placeholder="Email-Address:example@mail.com" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" 
